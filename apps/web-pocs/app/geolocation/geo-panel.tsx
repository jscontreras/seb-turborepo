...
export default function GeoPanel() {
  ...
  return (
    <Card className="p-4">
      ...
      <CardContent>
        {error ? <div className="text-destructive mb-4">{error}</div> : null}

        <div className="flex items-center gap-4">
          <MapPin className="h-8 w-8 text-primary" />
          <div>
            <p className="text-xl font-medium">{location?.city || "Dallas"}</p>
            <p className="text-muted-foreground">{location?.country || "USA"}</p>
            {isFallback && (
              <p className="text-xs text-muted-foreground mt-2">
                (Using fallback location - Vercel Edge Network geolocation is only available in production)
              </p>
            )}
          </div>
        </div>
