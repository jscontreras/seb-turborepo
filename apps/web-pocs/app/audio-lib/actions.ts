...
export async function updateAudioTitle(id: string, newTitle: string): Promise<AudioMetadata> {
  try {
    const audioData = await redis.hgetall(`${REDIS_KEYS.AUDIO_PREFIX}${id}`)
    ...
    // Update the title
    await redis.hset(`${REDIS_KEYS.AUDIO_PREFIX}${id}`, { title: newTitle.trim() })
  ...
  try {
    ...
    for (const id of audioIds) {
      ...
      if (audioData && audioData.blobPathname) {
        ...
        const matchingBlob = blobs.find((blob) => blob.pathname === audioData.blobPathname)
